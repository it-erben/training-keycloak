...
- Bind-Passwort des Teams in `lab/secrets/bind.pw`, angelegt mit
...
```bash
docker compose exec -e TEAM=01 keycloak bash /workshop/solution/apply-solution.sh <befehl>
```
...
| `users-dn users`     | Users DN zurück auf `OU=Users,OU=Team01,...`                                     |
| `users-dn team`      | Users DN auf die Team-OU, findet auch `OU=Moved`                                 |
...
| Bind type / Bind DN     | simple / `t01.bind@ad.mustertech.test`                            |
| Edit mode               | READ_ONLY                                                         |
...
Group-Mapper `ad-groups`: Groups DN `OU=Groups,OU=Team01,...`, Group Name LDAP Attribute `cn`,
