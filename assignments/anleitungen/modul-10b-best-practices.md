...
services:
  traefik:
    image: traefik:v3.0
    container_name: mustertech-traefik
    command:
      - "--api.insecure=true"
...
Für lokales Testen füge zu `/etc/hosts` (Linux/Mac) oder `C:\Windows\System32\drivers\etc\hosts` (Windows) hinzu:
...
Oder Initial-Admin per Umgebungsvariable nur beim ersten Start setzen, dann deaktivieren.
...
```bash
docker exec mustertech-keycloak /opt/keycloak/bin/kc.sh export \
...
```bash
# PostgreSQL Backup
docker exec mustertech-postgres pg_dump -U keycloak keycloak > backup/db_backup.sql

# Restore
cat backup/db_backup.sql | docker exec -i mustertech-postgres psql -U keycloak keycloak
```
...
## Teil 7: Checkliste für Produktion
...
- [ ] Produktionsmodus aktiviert (`start` statt `start-dev`)
- [ ] Datenbank-Verbindungspool konfiguriert
- [ ] Caching aktiviert
...
- Keycloak installiert und konfiguriert
- User, Gruppen und Rollen verwaltet
- Sicherheitsfunktionen implementiert
- Authentication Flows angepasst
- Eine React-Anwendung mit OIDC angebunden
- Identity Provider integriert
- Mehrere Client-Typen implementiert
- Authorization Services genutzt
- Theming durchgeführt
- APIs genutzt
- Best Practices für Produktion kennengelernt

**Du bist jetzt bereit, Keycloak in deinen Projekten einzusetzen!**
