...
- [ ] Modul 04 abgeschlossen (User existieren)
- [ ] Smartphone mit Authenticator-App (getestet mit Google Authenticator und OTP Auth)

> **Hinweis:** Zu Beginn dieser Übung richten wir zuerst OTP für max.admin ein, bevor wir den Custom Flow erstellen.
...
## Teil 5: Weitere Flow-Anpassungen

### Aufgabe 5.1: Login mit E-Mail erlauben bzw. verbieten

Standardmäßig muss der Username eingegeben werden. Um auch E-Mail zu erlauben oder zu verbieten:

Realm settings → Login → "Login with email" aktivieren/deaktivieren.

### Aufgabe 5.2: Registrierungs-Flow erkunden (Optional)

1. Navigiere zu **Authentication** → **registration**
2. Analysiere die Schritte. Wir können hier z.B. Captcha aktivieren

### Aufgabe 5.3: Required Actions verstehen
...
User loggt sich ein
    │
    ├─ Hat Session-Cookie? → Direkt eingeloggt
    │
    └─ Kein Cookie → Username/Passwort Formular
                          │
                          └─ Erfolgreich?
                                  │
                                  ├─ User hat Rolle "admin" oder OTP aktiv?
                                  │       │
                                  │       ├─ Ja → OTP abfragen
                                  │       │
                                  │       └─ Nein → Eingeloggt
